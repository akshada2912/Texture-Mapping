    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect) {
                si = siIntermediate;
                ray.t = si.t;
                si.v1=this->tris[this->getIdx(i + node.firstPrim)].v1,
                si.v2=this->tris[this->getIdx(i + node.firstPrim)].v2,
                si.v3=this->tris[this->getIdx(i + node.firstPrim)].v3,
                si.uv1=this->tris[this->getIdx(i + node.firstPrim)].uv1;
                si.uv2=this->tris[this->getIdx(i + node.firstPrim)].uv2;
                si.uv3=this->tris[this->getIdx(i + node.firstPrim)].uv3;
            }
        }
    }
    else {
