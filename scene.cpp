...
{
    // Output
    this->config=sceneConfig; //setting the sceneConfig to load lights
    try {
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            if (siIntermediate.t <= ray.t) {
                si = siIntermediate;
                ray.t = si.t;
                si.s_ind=this->getIdx(i + node.firstPrim);
                if(this->surfaces[this->getIdx(i + node.firstPrim)].hasDiffuseTexture())
                si.hasres=1;
                si.resolution.x=this->surfaces[this->getIdx(i + node.firstPrim)].diffuseTexture.resolution.x;
                si.resolution.y=this->surfaces[this->getIdx(i + node.firstPrim)].diffuseTexture.resolution.y;
                
              //  si.srface=this->surfaces[this->getIdx(i + node.firstPrim)];
            }
        }
    }
    else {
